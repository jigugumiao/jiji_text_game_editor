...
assert.match(html, /id="app-version">v25\.4\.110/, '界面版本必须标记本次合并');
...
assert.match(html, /js\/editor\.js\?v=20260917-01/, 'Agent 光标/选区上下文后必须刷新 editor.js 缓存标识');
assert.match(html, /js\/storage\.js\?v=20260917-01/, '合并 NS 隔离后必须刷新 storage.js 缓存标识');
assert.match(html, /css\/style\.css\?v=20260917-01/, 'Agent 多对话侧栏与弱提醒样式后必须刷新 style.css 缓存标识');
