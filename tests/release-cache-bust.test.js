...
assert.match(html, /id="app-version">v25\.4\.98</, '界面版本必须标记本次合并');
...
assert.match(html, /js\/editor\.js\?v=20260912-03/, '删除安全阀影响面报告后必须刷新 editor.js 缓存标识');
assert.match(html, /js\/storage\.js\?v=20260912-01/, '合并 NS 隔离后必须刷新 storage.js 缓存标识');
assert.match(html, /css\/style\.css\?v=20260912-02/, '删除确认危险警告样式后必须刷新 style.css 缓存标识');
assert.match(html, /js\/ai\.js\?v=20260912-01/, 'callDeepseek usage 透出后必须刷新 ai.js 缓存标识');
assert.match(html, /js\/agent\.js\?v=20260912-04/, '可靠性铁律与事实导向删除提示后必须刷新 agent.js 缓存标识');
