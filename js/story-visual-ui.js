...
(function () {
  ...
  function effectNaturalText(effect, states) {
    var source = effect || {}, types = stateTypes(states), name = String(source.name || '').trim(), type = types[name];
    if (!name || !type) return '请先选择要变化的变量。';
    var value = source.value !== undefined ? source.value : source.val;
    if (allowedOperations(type).indexOf(source.op) < 0 || value === '' || value === null || value === undefined || (type === 'number' && !/^-?\d+(\.\d+)?$/.test(String(value)))) return '请完成这条变量变化。';
    var change = name + ({ '+': '增加', '-': '减少', '=': '设为' }[source.op] || '') + ' ' + (type === 'boolean' ? (value === true || value === 'true' ? '是' : '否') : value) + '。';
    if (!source.condition) return '选中后，' + change;
    var condition = conditionNaturalText(source.condition, types);
    if (condition.indexOf('请完成第 ') === 0) return condition.replace('请完成第 ', '请完成这条变化的第 ');
    if (condition.indexOf('请先') === 0) return condition;
    return '当' + condition + '时，' + change;
  }
  ...
  function renderOptionEditor(host, node, initialDraft, context) {
    ...
    draft.effects.forEach(function (effect, index) {
      ...
      name.addEventListener('change', function () { effect.name = name.value; effect.op = allowedOperations(types[name.value])[0] || '='; effect.value = types[name.value] === 'boolean' ? true : ''; rerender(); });
      line.appendChild(name);
      ...
      if (name.value) {
        ...
        op.addEventListener('change', function () { effect.op = op.value; rerender(); }); value.addEventListener('input', function () { effect.value = inputValueForType(types[name.value], value); updateEffectSummary(index); }); value.addEventListener('change', function () { effect.value = types[name.value] === 'boolean' ? value.value === 'true' : inputValueForType(types[name.value], value); updateEffectSummary(index); });
        line.append(op, value);
