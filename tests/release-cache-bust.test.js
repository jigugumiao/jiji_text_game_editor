...
assert.match(html, /id="app-version">v25\.4\.111/, '界面版本必须标记本次合并');
...
assert.match(html, /js\/exporter\.js\?v=20260917-01/, '合并变量运行时后必须刷新 exporter.js 缓存标识');
assert.match(html, /js\/editor\.js\?v=20260917-02/, '恢复 Galgame 九宫格预设后必须刷新 editor.js 缓存标识');
assert.match(html, /js\/storage\.js\?v=20260917-01/, '合并 NS 隔离后必须刷新 storage.js 缓存标识');
assert.match(html, /js\/galgame-dialogue\.js\?v=20260917-01/, 'Galgame 九宫格预设共享模块必须带缓存标识');
assert.match(html, /css\/style\.css\?v=20260917-02/, '恢复 Galgame 九宫格预设样式后必须刷新 style.css 缓存标识');
